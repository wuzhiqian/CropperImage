...
public class CropImageView extends FrameLayout {
    ...
    public static boolean CROP_IMG_TYPE_SQU = true;//是不是要切正方形
    ...
    public static boolean isCropImgTypeSqu() {
        return CROP_IMG_TYPE_SQU;
    }

    public static void setCropImgTypeSqu(boolean cropImgTypeSqu) {
        CROP_IMG_TYPE_SQU = cropImgTypeSqu;
    }
    ...
    public Bitmap getCroppedImage() {
        ...
        if (actualCropY < 0)
            actualCropY = 0;
        if (actualCropWidth > mBitmap.getWidth())
            actualCropWidth =  mBitmap.getWidth();
        if(actualCropHeight > mBitmap.getHeight())
            actualCropHeight = mBitmap.getHeight();
