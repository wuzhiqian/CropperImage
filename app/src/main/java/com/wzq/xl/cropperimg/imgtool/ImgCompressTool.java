...
public class ImgCompressTool {
    ...
    public static ImgCompressTool instance = null;
    public static ImgCompressTool getInstance()
    {
        if(instance == null)
            instance = new ImgCompressTool();
        return instance;
    }
    ...
    public void compressAndGenImage(Bitmap image, String outPath, int maxSize) throws IOException {;
        ...
        if (os.toByteArray().length / 1024 > maxSize) {
            // Clean up os
            os.reset();
            if(os.toByteArray().length / 1024  > 1000)
                image.compress(Bitmap.CompressFormat.JPEG,10, os);
            else if(os.toByteArray().length / 1024 > 500)
...
//            image.compress(Bitmap.CompressFormat.JPEG, 50, os);
    ...
    public Uri compressAndGenImageRetUrl(Bitmap image, String outPath, int maxSize)  {
        try {
            compressAndGenImage(image, outPath, maxSize);
            return Uri.fromFile(new File(outPath));
        } catch (IOException e) {
            e.printStackTrace();
        }
        return null;
